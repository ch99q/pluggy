...
export default createPlatform((ctx) => ({
  ...
  async getLatestVersion() {
    ...
    if (!latestVersion) throw new Error("No versions found for Bukkit");
  ...
  async download(version, ignoreCache = false) {
    ...
    const JAR_PATH = join(
      CACHE_PATH,
      "versions",
      `craftbukkit-${version.version}-${version.build}.jar`,
    );
    ...
    const compiler = await compile(ctx, version.version, "craftbukkit", ignoreCache);
